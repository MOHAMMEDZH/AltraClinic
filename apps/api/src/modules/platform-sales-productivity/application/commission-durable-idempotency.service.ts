...
import { SalesProductivityError } from '../domain/sales-productivity.errors';
import { SALES_COMMISSION_IDEMPOTENCY_RETENTION_MS } from '../platform-sales-productivity.constants';
...
export class CommissionIdempotencyEquivalentRaceLostError extends Error {
  constructor(
    readonly resultResourceType: string,
    readonly resultResourceId: string,
    readonly resultPayload?: unknown,
  ) {
    super('Equivalent commission idempotency race lost — reload original result.');
    this.name = 'CommissionIdempotencyEquivalentRaceLostError';
  }
}
...
/**
 * Flexible Step 26 — durable claim/result store for commission snapshot mutations.
 * Reuses `platform_sales_idempotency` with operation namespace `sales_commission.*`.
 */
@Injectable()
export class CommissionDurableIdempotencyService {
  ...
  assertValidKey(idempotencyKey: string): string {
    ...
    if (!key || !IDEMPOTENCY_KEY_REGEX.test(key)) {
      throw new SalesProductivityError('idempotency_required', 'Invalid Idempotency-Key', 400);
    }
    return key;
  }
  ...
  async claimOrReplay(input: {
    ...
    | { kind: 'replay'; result: CommissionActionResult }
    | { kind: 'proceed'; idempotencyKey: string; requestHash: string; claimId: string }
  > {
    const key = this.assertValidKey(input.idempotencyKey);
    const now = new Date();
    const expiresAt = new Date(Date.now() + SALES_COMMISSION_IDEMPOTENCY_RETENTION_MS);
  ...
  async completeInTransaction(
    ...
    throw new CommissionIdempotencyEquivalentRaceLostError(
  ...
  payloadToResult(payload: unknown): CommissionActionResult {
    const p = payload as CommissionActionResult | null;
    if (!p || typeof p !== 'object' || typeof p.action !== 'string') {
      throw new SalesProductivityError(
        'idempotency_conflict',
        'Durable claim missing result payload',
        409,
      );
    }
    return {
