...
/**
 * Flexible Step 26 — Step 21 Model A durable audit writer for commission snapshots.
 * Sentinel tenant scoped; no PHI, no secrets, no invented commission amounts.
 */
@Injectable()
export class CommissionAuditLog {
  ...
  async record(entry: CommissionAuditRecord): Promise<string> {
    return this.prisma.withPlatformBypass((client) => this.recordInTransaction(client, entry));
  }

  async recordInTransaction(
    client: Prisma.TransactionClient,
    entry: CommissionAuditRecord,
  ): Promise<string> {
    if (isSalesProductivityFailureInjectionActive('after_audit_staging_before_commit')) {
      throw new SalesProductivityError('injected_failure', 'Injected after audit staging', 500);
    }
