...
import { SALES_TRIAL_IDEMPOTENCY_RETENTION_MS } from '../platform-sales-trials.constants';
...
export class TrialIdempotencyEquivalentRaceLostError extends Error {
  constructor(
    readonly resultResourceType: string,
    readonly resultResourceId: string,
    readonly resultPayload?: unknown,
  ) {
    super('Equivalent trial idempotency race lost — reload original result.');
    this.name = 'TrialIdempotencyEquivalentRaceLostError';
  }
}
...
/**
 * Flexible Step 25 — durable claim/result store for Trial mutations.
 * Reuses `platform_sales_idempotency` with operation namespace `sales_trial.*`.
 * Never process-local: multi-instance replay must observe the same durable row.
 */
@Injectable()
export class TrialDurableIdempotencyService {
  ...
  assertValidKey(idempotencyKey: string): string {
    ...
    if (!key || !IDEMPOTENCY_KEY_REGEX.test(key)) {
      throw new SalesTrialError('idempotency_required', 'Invalid Idempotency-Key', 400);
    }
    return key;
  }
  ...
  > {
    ...
    throw new SalesTrialError('idempotency_conflict', 'Idempotency claim could not be acquired', 409);
  }
  ...
  async completeInTransaction(
    ...
    throw new TrialIdempotencyEquivalentRaceLostError(
  ...
  payloadToResult(payload: unknown): TrialActionResult {
    const p = payload as TrialActionResult | null;
    if (!p || typeof p !== 'object' || typeof p.action !== 'string') {
      throw new SalesTrialError('idempotency_conflict', 'Durable claim missing result payload', 409);
    }
    return {
