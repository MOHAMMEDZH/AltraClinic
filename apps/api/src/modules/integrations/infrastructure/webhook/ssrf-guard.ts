/**
 * SSRF guard for Integrations webhooks (Phase 44c).
 * Pattern aligned with Notification delivery; owned by Integrations module — does not modify Phase 41.
 */
...
export function evaluateWebhookUrlSafety(rawUrl: string): UrlSafetyResult {
  ...
  if (url.protocol !== 'https:') {
    return {
      safe: false,
      reason: `protocol "${url.protocol}" is not allowed; HTTPS only`,
  ...
  if (hostname.endsWith('.local') || hostname.endsWith('.internal')) {
    return {
      safe: false,
      reason: `hostname "${hostname}" is a reserved internal TLD`,
      hostname,
    };
  }
  ...
  if (hostname.includes(':') && isPrivateOrReservedIpv6(hostname)) {
    return {
      safe: false,
      reason: `IP "${hostname}" is a private/reserved IPv6 address`,
      hostname,
    };
  }
