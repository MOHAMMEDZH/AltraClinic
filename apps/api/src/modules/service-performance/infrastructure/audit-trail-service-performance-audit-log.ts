...
import {
  ...
  ServicePerformanceAuditRecord,
} from '../ports/service-performance-audit-log.port';

@Injectable()
export class AuditTrailServicePerformanceAuditLog implements ServicePerformanceAuditLog {
  private readonly factory = new AuditEntryFactory();
  constructor(private readonly prisma: PrismaService) {}

  async record(entry: ServicePerformanceAuditRecord): Promise<void> {
    await this.prisma.withPlatformBypass(async (client) => this.writeAudit(client, entry));
  }

  async recordInTransaction(client: unknown, entry: ServicePerformanceAuditRecord): Promise<void> {
    await this.writeAudit(client as Prisma.TransactionClient, entry);
  }

  private async writeAudit(client: Prisma.TransactionClient, entry: ServicePerformanceAuditRecord) {
    const actorRoles = entry.actorRoles?.length ? entry.actorRoles : ['service_performance'];
    const detailsForFactory = entry.details
      ? Object.fromEntries(
          Object.entries(entry.details).map(([k, v]) => [k, v == null ? '' : String(v)]),
        )
      : null;
