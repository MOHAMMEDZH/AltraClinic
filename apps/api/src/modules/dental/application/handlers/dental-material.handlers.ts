...
@Injectable()
export class ListDentalProcedureMaterialsHandler {
  constructor(
  ...
  async execute(procedureCode: string) {
    ...
    const rows = await this.prisma.dentalProcedureMaterial.findMany({
...
@Injectable()
export class CreateDentalProcedureMaterialHandler {
  constructor(
    private readonly prisma: PrismaService,
    private readonly tenantContext: TenantContextService,
  ) {}
  ...
  async execute(input: {
    ...
    const row = await this.prisma.dentalProcedureMaterial.upsert({
      where: {
...
@Injectable()
export class ListPatientDentalMaterialsHandler {
  constructor(
...
@Injectable()
export class ConsumeDentalMaterialHandler {
  constructor(
    @Inject(PATIENT_REPOSITORY) private readonly patientRepo: PatientRepository,
    private readonly tenantContext: TenantContextService,
  ...
  async execute(
    ...
    const reason = procedureCode ? `Dental ${procedureCode}` : 'Dental procedure';
...
@Injectable()
export class SearchDentalClinicalInventoryHandler {
  constructor(
    private readonly tenantContext: TenantContextService,
