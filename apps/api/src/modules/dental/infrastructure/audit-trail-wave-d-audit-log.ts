...
import { WAVE_D_AUDIT_LOG, WaveDAuditLog, WaveDAuditRecord } from '../ports/wave-d-audit-log.port';
...
@Injectable()
export class AuditTrailWaveDAuditLog implements WaveDAuditLog {
  private readonly factory = new AuditEntryFactory();

  constructor(private readonly prisma: PrismaService) {}

  async record(entry: WaveDAuditRecord): Promise<void> {
    await this.prisma.withPlatformBypass(async (client) => {
      await this.writeAudit(client, entry);
    });
  }

  async recordInTransaction(client: unknown, entry: WaveDAuditRecord): Promise<void> {
    await this.writeAudit(client as Prisma.TransactionClient, entry);
  }

  private async writeAudit(client: Prisma.TransactionClient, entry: WaveDAuditRecord) {
    const actorRoles = entry.actorRoles?.length > 0 ? entry.actorRoles : ['dental'];
