/**
 * Flexible Step 27 â€” template catalog / rendering matrix T01â€“T18.
 * Code-defined catalog (no CMS) â€” see `platform-template.catalog.ts`.
 */
...
describeDb('Step 27 template catalog matrix TMP01-TMP18 (PostgreSQL)', () => {
  ...
  it('TMP01: catalog contains exactly 24 code-defined templates', () => {
    expect(PLATFORM_TEMPLATES).toHaveLength(24);
    expect(listPlatformTemplates()).toHaveLength(24);
  });

  it('TMP02: every template requiredVariables is a subset of its declared variables', () => {
  ...
  it('TMP03: every template declares both en-US and ar-SY locale content', () => {
    for (const tpl of PLATFORM_TEMPLATES) {
  ...
  it('TMP04: listTemplates via query service requires templates.view permission', () => {
  ...
  it('TMP05: getTemplate returns full definition including locales for a known key', () => {
  ...
  it('TMP06: getTemplate throws template_not_found for an unknown key', () => {
    expect(() => getPlatformTemplateByKey('tpl.platform.does_not_exist')).toThrow(/Unknown template/i);
  });

  it('TMP07: preview() renders synthetic variables without leaking real data and records an audit entry', () => {
  ...
  it('TMP08: preview() without templates.view permission is forbidden', () => {
    const user = platformClaims(randomUUID(), randomUUID());
  ...
  it('TMP09: viewer permission set (read-only) can list and preview templates', () => {
  ...
  it('TMP10: renderPlatformTemplate rejects forbidden variable names (PHI/secret guard)', () => {
  ...
  it('TMP11: renderPlatformTemplate rejects a genuinely undeclared variable with a non-null value', () => {
    const tpl = getPlatformTemplateByKey('tpl.platform.invitation.sent');
    expect(() =>
      renderPlatformTemplate(tpl, 'en-US', {
        recipientDisplayName: 'A',
  ...
  it('TMP12: renderPlatformTemplate tolerates a null/undefined value for an undeclared key', () => {
    const tpl = getPlatformTemplateByKey('tpl.platform.invitation.sent');
    expect(() =>
      renderPlatformTemplate(tpl, 'en-US', {
        recipientDisplayName: 'A',
  ...
  it('TMP13: renderPlatformTemplate throws missing_variable when a required variable is absent', () => {
    const tpl = getPlatformTemplateByKey('tpl.platform.invitation.sent');
    expect(() =>
      renderPlatformTemplate(tpl, 'en-US', {
        recipientDisplayName: 'A',
  ...
  it('TMP14: ar-SY locale renders Arabic subject/body distinct from en-US', () => {
    const tpl = getPlatformTemplateByKey('tpl.platform.invitation.sent');
  ...
  it('TMP15: HTML-bearing variable values are escaped in the rendered output (XSS guard)', () => {
    const tpl = getPlatformTemplateByKey('tpl.platform.invitation.sent');
  ...
  it('TMP16: template_lookup failure injection blocks getPlatformTemplateByKey', () => {
  ...
  it('TMP17: template_renderer failure injection blocks rendering for any locale', () => {
    const tpl = getPlatformTemplateByKey('tpl.platform.invitation.sent');
  ...
  it('TMP18: locale_renderer failure injection blocks only ar-SY, not en-US', () => {
    const tpl = getPlatformTemplateByKey('tpl.platform.invitation.sent');
