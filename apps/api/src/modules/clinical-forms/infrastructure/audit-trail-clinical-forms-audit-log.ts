...
@Injectable()
export class AuditTrailClinicalFormsAuditLog implements ClinicalFormsAuditLog {
  private readonly factory = new AuditEntryFactory();

  constructor(private readonly prisma: PrismaService) {}

  async record(entry: ClinicalFormsAuditRecord): Promise<void> {
  ...
  private async writeAudit(
    ...
    const actorRoles =
      entry.actorRoles?.length > 0 ? entry.actorRoles : ['clinical_forms'];
