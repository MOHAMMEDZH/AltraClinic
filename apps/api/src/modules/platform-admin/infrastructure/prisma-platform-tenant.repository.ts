...
export class PrismaPlatformTenantRepository implements PlatformTenantRepository {
  ...
  async save(platformTenant: PlatformTenant): Promise<void> {
    ...
    // Region and plan domain values may differ in casing from Prisma enum — use assertion
    const prismaRegion = platformTenant.region.value.toUpperCase().replace(/-/g, '_') as unknown as Prisma.PlatformTenantCreateInput['region'];
    const prismaPlan =
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.platformTenant.upsert({
  ...
  private toDomain(row: PrismaPlatformTenantRow): PlatformTenant {
    ...
    return PlatformTenant.restore({
      ...
      region: new PlatformRegionVO(row.region),
