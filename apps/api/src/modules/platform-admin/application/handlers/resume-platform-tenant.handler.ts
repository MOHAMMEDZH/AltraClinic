...
import { ResumePlatformTenantCommand } from '../commands/platform-admin.commands';
...
import { PlatformTenantResumedEvent } from '../../domain/events/platform-tenant-resumed.event';
import { resolveOperatorLocale } from './resolve-operator-locale.util';

@Injectable()
export class ResumePlatformTenantHandler {
  constructor(
  ...
  async execute(command: ResumePlatformTenantCommand): Promise<void> {
    ...
    if (!this.policy.canManageTenantLifecycle(command.actorRoles)) {
      throw new ForbiddenException('User does not have permission to resume platform tenants');
    }
    ...
    platformTenant.resume();
    await this.repository.save(platformTenant);

    await this.eventPublisher.publish(
      new PlatformTenantResumedEvent(platformTenant.tenantId, platformTenant.id, command.actorId),
    );

    await this.auditLog.record({
      tenantId: platformTenant.tenantId,
      action: 'platform_admin.tenant.resumed',
      resourceId: platformTenant.id,
      actorId: command.actorId,
      actorRoles: command.actorRoles,
      locale: await resolveOperatorLocale(this.tenantContext),
      reason: null,
      descriptionEn: `Resumed tenant ${platformTenant.displayName}`,
      descriptionAr: `تم استئناف المستأجر ${platformTenant.displayName}`,
