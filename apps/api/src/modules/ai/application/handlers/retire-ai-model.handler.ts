...
import { RetireAiModelCommand } from '../commands/retire-ai-model.command';
...
import { AiModelRetiredEvent } from '../../domain/events/ai-model-retired.event';
...
@Injectable()
export class RetireAiModelHandler {
  constructor(
  ...
  async execute(command: RetireAiModelCommand): Promise<void> {
    ...
    if (!command.retiredBy?.trim()) {
      throw new BadRequestException('RetiredBy is required');
    }
    if (!this.policy.canOperateModels(command.retiredByRoles ?? [])) {
      throw new ForbiddenException('User does not have permission to retire AI models');
    }
    ...
    model.retire(command.retiredBy);
    await this.repository.save(model);
    await this.eventPublisher.publish(new AiModelRetiredEvent(tenantId, model.id, command.retiredBy));
  }
}
