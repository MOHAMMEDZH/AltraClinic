#!/usr/bin/env node
/**
 * Flexible Step 21 — clean migration validator.
 * Fresh DB: migrate deploy → Catalog 68/136/68/13 → export tables empty → no billing → no Step 22.
 *
...
const cleanDb = `test_ac_clean_${Date.now()}`;
...
const STEP22_FORBIDDEN = [
  'platform_operations_console',
  'platform_ops_job_controls',
  'platform_backup_control_plane',
];
...
async function main() {
  ...
  try {
    ...
    for (const table of STEP21_TABLES) {
      const rows = await prisma.$queryRawUnsafe(
        `SELECT to_regclass('public.${table}') IS NOT NULL AS present`,
      );
      if (!rows[0]?.present) throw new Error(`Missing Step 21 table: ${table}`);
    }

    for (const table of [...BILLING_FORBIDDEN, ...STEP22_FORBIDDEN]) {
      const rows = await prisma.$queryRawUnsafe(
        `SELECT to_regclass('public.${table}') IS NOT NULL AS present`,
      );
    ...
    console.log('OK no billing/Step 22 schema tables');
    console.log('OK zero automatic exports / invented audits from migration');
    log(`Step 21 clean migration validator passed (db=${cleanDb}).`);
  } finally {
    try {
      await prisma.$disconnect();
    } catch {
