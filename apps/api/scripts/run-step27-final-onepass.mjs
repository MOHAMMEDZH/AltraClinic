#!/usr/bin/env node
/**
 * Flexible Step 27 — one uninterrupted final acceptance sequence orchestrator.
 * Stops on first non-zero exit. Does not retry. Does not implement Step 28/29.
 */
...
const logPath = path.join(apiRoot, 'step27-final-onepass.jsonl');
...
function freezeIntegrationDatabase() {
  ...
  env.STEP27_FROZEN_DATABASE_URL = url;
  env.STEP27_FROZEN_DATABASE_NAME = dbName;
  env.STEP27_FROZEN_AT = frozenAt;
  fs.appendFileSync(
    logPath,
    JSON.stringify({
...
function assertFrozenDatabaseUnchanged(label) {
  if (env.INTEGRATION_DATABASE_URL !== env.STEP27_FROZEN_DATABASE_URL) {
    console.error(
...
console.log('Step 27 final one-pass sequence starting', {
  start: seqStart.toISOString(),
  branch,
  commit,
  node: process.version,
...
// Step 27 validators: preferences present and empty; Catalog 68/136/68/13; no second engine / Step 28
...
// 26e Step 27 full runner (N/TW/L/I/R/T/A/F/C/H/P + query/index + window.util)
...
fs.appendFileSync(
  ...
  JSON.stringify({
    ...
    frozenDatabase: env.STEP27_FROZEN_DATABASE_NAME,
    frozenDatabaseUrl: env.STEP27_FROZEN_DATABASE_URL,
  }) + '\n',
);

console.log('\nStep 27 FINAL ONE-PASS SEQUENCE COMPLETE', {
  durationMs: seqEnd - seqStart,
  logPath,
  frozenDatabase: env.STEP27_FROZEN_DATABASE_NAME,
});
