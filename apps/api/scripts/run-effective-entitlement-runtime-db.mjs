#!/usr/bin/env node
/**
 * Runs Step 17 Effective Entitlement Runtime PostgreSQL integration suites.
 */
...
async function ensurePublishedPlanFixture() {
  ...
  try {
    ...
    if (published > 0) {
      console.log(`Step 17 preflight: ${published} published Plan Version(s) present.`);
      return;
    }
    console.log('Step 17 preflight: reseeding Plans with commercial definitions...');
    ...
    if (fs.existsSync(seedScript)) {
      ...
      if (seed.status !== 0) {
        throw new Error('Failed to seed commercial plan fixtures for Step 17.');
      }
    } else {
...
const result = spawnSync(
  ...
  [
    ...
    apiRoot,
    '--testPathPattern',
    'effective-entitlement-runtime/.*\\.spec\\.ts$',
  ],
