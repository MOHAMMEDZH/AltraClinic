#!/usr/bin/env node
/**
 * Flexible Step 23 — one uninterrupted final acceptance sequence orchestrator.
 * Stops on first non-zero exit. Does not retry. Does not implement Step 24.
 */
...
const logPath = path.join(apiRoot, 'step23-final-onepass.jsonl');
...
function freezeIntegrationDatabase() {
  ...
  env.STEP23_FROZEN_DATABASE_URL = url;
  env.STEP23_FROZEN_DATABASE_NAME = dbName;
  env.STEP23_FROZEN_AT = frozenAt;
  fs.appendFileSync(
    logPath,
    JSON.stringify({
...
function assertFrozenDatabaseUnchanged(label) {
  if (env.INTEGRATION_DATABASE_URL !== env.STEP23_FROZEN_DATABASE_URL) {
    console.error(
...
console.log('Step 23 final one-pass sequence starting', {
  start: seqStart.toISOString(),
  branch,
  commit,
  node: process.version,
...
// 3–8 Step 21/22 validators (baseline still holds under Step 23 schema)
...
// 26 Step 23 full runner (A01-A16 / S01-S16 / M01-M08 / C01-C24 / F01-F30 / H01-H48)
...
fs.appendFileSync(
  ...
  JSON.stringify({
    ...
    frozenDatabase: env.STEP23_FROZEN_DATABASE_NAME,
    frozenDatabaseUrl: env.STEP23_FROZEN_DATABASE_URL,
  }) + '\n',
);

console.log('\nSTEP 23 FINAL ONE-PASS SEQUENCE COMPLETE', {
  durationMs: seqEnd - seqStart,
  logPath,
  frozenDatabase: env.STEP23_FROZEN_DATABASE_NAME,
});
