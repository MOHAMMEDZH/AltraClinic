#!/usr/bin/env node
/**
 * Flexible Step 20 — one uninterrupted final acceptance sequence orchestrator.
 * Stops on first non-zero exit. Does not retry. Does not implement Step 21.
 */
...
const logPath = path.join(apiRoot, 'step20-final-onepass.jsonl');
...
const env = {
  ...
  // Containment defaults OFF for the sequence; Step 20 runner enables explicitly.
...
function waitPostgres(label) {
  ...
  meta(
    ...
    ended,
    ready ? 0 : 1,
    ready ? 'postgres_ready' : 'postgres_not_ready',
  );
  if (!ready) {
...
console.log('Step 20 final one-pass sequence starting', {
  start: seqStart.toISOString(),
  branch,
  commit,
  node: process.version,
...
// 3–4 Step 20 validators
run('npm run test:feature-flags-settings-clean-migration', apiRoot);
run('npm run test:feature-flags-settings-upgrade-migration', apiRoot);
waitPostgres('post-validators');
// 5–14 cross-domain through Step 17
const dbSuites = [
...
// 20–23 Super Admin
run('npx vitest run src/pages/feature-flags-settings.spec.tsx src/i18n/messages.parity.spec.ts', saRoot);
...
console.log('\nSTEP 20 FINAL ONE-PASS SEQUENCE COMPLETE', {
  durationMs: seqEnd - seqStart,
  logPath,
});
process.exit(0);
