#!/usr/bin/env node
/**
 * Flexible Step 25 — one uninterrupted final acceptance sequence orchestrator.
...
const logPath = path.join(apiRoot, 'step25-final-onepass.jsonl');
...
function freezeIntegrationDatabase() {
  ...
  env.STEP25_FROZEN_DATABASE_URL = url;
  env.STEP25_FROZEN_DATABASE_NAME = dbName;
  env.STEP25_FROZEN_AT = frozenAt;
  fs.appendFileSync(
    logPath,
    JSON.stringify({
...
function assertFrozenDatabaseUnchanged(label) {
  if (env.INTEGRATION_DATABASE_URL !== env.STEP25_FROZEN_DATABASE_URL) {
    console.error(
...
console.log('Step 25 final one-pass sequence starting', {
  start: seqStart.toISOString(),
  branch,
  commit,
  node: process.version,
...
// Step 25 validators: trial tables present and empty, no Step 26, no auto-created trials
...
// 30–33 Super Admin
run(
  'npx vitest run src/pages/audit-center.spec.tsx src/pages/operations-console.spec.tsx src/pages/sales/trials/sales-trials-ui.spec.tsx src/i18n/messages.parity.spec.ts',
  saRoot,
);
...
fs.appendFileSync(
  ...
  JSON.stringify({
    ...
    frozenDatabase: env.STEP25_FROZEN_DATABASE_NAME,
    frozenDatabaseUrl: env.STEP25_FROZEN_DATABASE_URL,
  }) + '\n',
);

console.log('\nStep 25 FINAL ONE-PASS SEQUENCE COMPLETE', {
  durationMs: seqEnd - seqStart,
  logPath,
  frozenDatabase: env.STEP25_FROZEN_DATABASE_NAME,
});
