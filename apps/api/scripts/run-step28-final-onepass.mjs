#!/usr/bin/env node
/**
 * Flexible Step 28 — Security Hardening final one-pass orchestrator.
 * Stops on first non-zero exit. Does not retry. Does not implement Step 29.
 */
...
const logPath = path.join(apiRoot, 'step28-final-onepass.jsonl');
...
function freezeIntegrationDatabase() {
  ...
  env.STEP28_FROZEN_DATABASE_URL = url;
  env.STEP28_FROZEN_DATABASE_NAME = dbName;
  env.STEP28_FROZEN_AT = frozenAt;
  fs.appendFileSync(
    logPath,
    JSON.stringify({
...
function assertFrozenDatabaseUnchanged(label) {
  if (env.INTEGRATION_DATABASE_URL !== env.STEP28_FROZEN_DATABASE_URL) {
    console.error(
...
console.log('Step 28 final one-pass sequence starting', {
  start: seqStart.toISOString(),
  branch,
  commit,
  node: process.version,
...
// 3–10 Step 21/22/23/24 validators (baseline still holds under later schema)
...
// Step 25 validators: trial tables present and empty
run('npm run test:sales-trials-clean-migration', apiRoot);
run('npm run test:sales-trials-upgrade-migration', apiRoot);
// Step 26 validators: commission snapshots present and empty
run('npm run test:sales-productivity-clean-migration', apiRoot);
run('npm run test:sales-productivity-upgrade-migration', apiRoot);
// Step 27 validators: preferences present and empty; Catalog 68/136/68/13; no Step 29
run('npm run test:notifications-templates-clean-migration', apiRoot);
run('npm run test:notifications-templates-upgrade-migration', apiRoot);
waitPostgres('post-validators');
// Step 28 security package (CORS/HDR/MA/RL/SEC/matrix registry)
run(
  'npx jest --config jest.config.cjs --runInBand --testPathPattern "step28-security-hardening.unit"',
  apiRoot,
);
...
// 26b Step 24 full runner
run('npm run test:sales-leads-db', apiRoot, {
  timeoutMs: longSuiteTimeoutMs,
});
waitPostgres('after-sales-leads-db');
// 26c Step 25 full runner
run('npm run test:sales-trials-db', apiRoot, {
  timeoutMs: longSuiteTimeoutMs,
});
waitPostgres('after-sales-trials-db');
// 26d Step 26 full runner
run('npm run test:sales-productivity-db', apiRoot, {
  timeoutMs: longSuiteTimeoutMs,
});
waitPostgres('after-sales-productivity-db');
// 26e Step 27 full runner (notification security regression / NOTSEC)
run('npm run test:notifications-templates-db', apiRoot, {
  timeoutMs: longSuiteTimeoutMs,
});
...
// 30–33 Super Admin
run(
  'npx vitest run src/pages/audit-center.spec.tsx src/pages/operations-console.spec.tsx src/pages/sales/trials/sales-trials-ui.spec.tsx src/pages/sales/productivity/sales-productivity-ui.spec.tsx src/pages/notifications/notifications-ui.spec.tsx src/i18n/messages.parity.spec.ts',
  saRoot,
);
...
fs.appendFileSync(
  ...
  JSON.stringify({
    ...
    frozenDatabase: env.STEP28_FROZEN_DATABASE_NAME,
    frozenDatabaseUrl: env.STEP28_FROZEN_DATABASE_URL,
  }) + '\n',
);

console.log('\nStep 28 FINAL ONE-PASS SEQUENCE COMPLETE', {
  durationMs: seqEnd - seqStart,
  logPath,
  frozenDatabase: env.STEP28_FROZEN_DATABASE_NAME,
});
