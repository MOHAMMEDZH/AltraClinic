#!/usr/bin/env node
/**
 * Flexible Step 21 — one uninterrupted final acceptance sequence orchestrator.
 * Stops on first non-zero exit. Does not retry. Does not implement Step 22.
 */
...
const logPath = path.join(apiRoot, 'step21-final-onepass.jsonl');
...
function freezeIntegrationDatabase() {
  ...
  env.STEP21_FROZEN_DATABASE_URL = url;
  env.STEP21_FROZEN_DATABASE_NAME = dbName;
  env.STEP21_FROZEN_AT = frozenAt;
  fs.appendFileSync(
    logPath,
    JSON.stringify({
...
function assertFrozenDatabaseUnchanged(label) {
  if (env.INTEGRATION_DATABASE_URL !== env.STEP21_FROZEN_DATABASE_URL) {
    console.error(
...
console.log('Step 21 final one-pass sequence starting', {
  start: seqStart.toISOString(),
  branch,
  commit,
  node: process.version,
...
// 23–26 Super Admin
run('npx vitest run src/pages/audit-center.spec.tsx src/i18n/messages.parity.spec.ts', saRoot);
...
fs.appendFileSync(
  ...
  JSON.stringify({
    ...
    frozenDatabase: env.STEP21_FROZEN_DATABASE_NAME,
    frozenDatabaseUrl: env.STEP21_FROZEN_DATABASE_URL,
  }) + '\n',
);

console.log('\nSTEP 21 FINAL ONE-PASS SEQUENCE COMPLETE', {
  durationMs: seqEnd - seqStart,
  logPath,
});
process.exit(0);
