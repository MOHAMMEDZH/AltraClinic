#!/usr/bin/env node
/**
 * Flexible Step 26 — one uninterrupted final acceptance sequence orchestrator.
 * Stops on first non-zero exit. Does not retry. Does not implement Step 26.
 */
...
const logPath = path.join(apiRoot, 'step26-final-onepass.jsonl');
...
function freezeIntegrationDatabase() {
  ...
  env.STEP26_FROZEN_DATABASE_URL = url;
  env.STEP26_FROZEN_DATABASE_NAME = dbName;
  env.STEP26_FROZEN_AT = frozenAt;
  fs.appendFileSync(
    logPath,
    JSON.stringify({
...
function assertFrozenDatabaseUnchanged(label) {
  if (env.INTEGRATION_DATABASE_URL !== env.STEP26_FROZEN_DATABASE_URL) {
    console.error(
...
console.log('Step 26 final one-pass sequence starting', {
  start: seqStart.toISOString(),
  branch,
  commit,
  node: process.version,
...
// 3–10 Step 21/22/23/24 validators (baseline still holds under Step 25 schema)
...
// Step 26 validators: commission snapshots present and empty; no payroll/Step 27
...
// 26c Step 25 full runner (create / extension / expiry / conversion / preview / governance)
run('npm run test:sales-trials-db', apiRoot, {
  timeoutMs: longSuiteTimeoutMs,
});
waitPostgres('after-sales-trials-db');
// 26d Step 26 full runner (metrics / snapshots / export / matrices)
...
// 30–33 Super Admin
run(
  'npx vitest run src/pages/audit-center.spec.tsx src/pages/operations-console.spec.tsx src/pages/sales/trials/sales-trials-ui.spec.tsx src/pages/sales/productivity/sales-productivity-ui.spec.tsx src/i18n/messages.parity.spec.ts',
  saRoot,
);
...
fs.appendFileSync(
  ...
  JSON.stringify({
    ...
    frozenDatabase: env.STEP26_FROZEN_DATABASE_NAME,
    frozenDatabaseUrl: env.STEP26_FROZEN_DATABASE_URL,
  }) + '\n',
);

console.log('\nStep 26 FINAL ONE-PASS SEQUENCE COMPLETE', {
  durationMs: seqEnd - seqStart,
  logPath,
  frozenDatabase: env.STEP26_FROZEN_DATABASE_NAME,
});
