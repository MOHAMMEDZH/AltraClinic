#!/usr/bin/env node
/**
 * Flexible Step 24 — Leads and Sales Pipeline clean migration validator.
 * Fresh DB: migrate deploy → Catalog 68/136/68/13 → Step 23+24 tables present & empty →
 * no billing → forbid trials/opportunities/pipeline_stages.
 */
...
const cleanDb = `test_sales24_clean_${Date.now()}`;
...
function log(msg) {
  console.log(`[sales24-clean-validator +${Date.now() - startedAt}ms] ${msg}`);
}
...
async function main() {
  ...
  try {
    ...
    for (const table of [...STEP23_TABLES, ...STEP24_TABLES]) {
    ...
    let items, translations, aliases, rules, salesReps, leads, stageHistory, ownershipHistory, notes;
    let lastCountErr;
    for (let i = 0; i < 10; i++) {
      assertWithinDeadline('catalog counts');
      try {
        [items, translations, aliases, rules, salesReps, leads, stageHistory, ownershipHistory, notes] =
          await Promise.all([
            ...
            prisma.platformSalesLeadStageHistory.count(),
            prisma.platformSalesLeadOwnershipHistory.count(),
            prisma.platformSalesLeadNote.count(),
          ]);
    ...
    expect('sales leads (no auto-created leads)', leads, 0);
    expect('sales lead stage history (empty)', stageHistory, 0);
