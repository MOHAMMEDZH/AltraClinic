#!/usr/bin/env node
/**
 * Flexible Step 22 — one uninterrupted final acceptance sequence orchestrator.
 * Stops on first non-zero exit. Does not retry. Does not implement Step 23.
 */
...
const logPath = path.join(apiRoot, 'step22-final-onepass.jsonl');
...
function freezeIntegrationDatabase() {
  ...
  env.STEP22_FROZEN_DATABASE_URL = url;
  env.STEP22_FROZEN_DATABASE_NAME = dbName;
  env.STEP22_FROZEN_AT = frozenAt;
  fs.appendFileSync(
    logPath,
    JSON.stringify({
...
function assertFrozenDatabaseUnchanged(label) {
  if (env.INTEGRATION_DATABASE_URL !== env.STEP22_FROZEN_DATABASE_URL) {
    console.error(
...
console.log('Step 22 final one-pass sequence starting', {
  start: seqStart.toISOString(),
  branch,
  commit,
  node: process.version,
...
// 5 Platform DB security
run('npm run test:platform-db-security', apiRoot);
// 6–8 auth/RBAC covered inside platform-db-security + later jest guards
// 9–14 Tenant Directory through Subscriptions / Step 18 / U01 surfaces
...
// 15 U01 is covered by usage-metering suite above when present
// 16 Step 17 covered by tenant-creation-provisioning
// 17 Step 19
...
// 23–26 Super Admin
run('npx vitest run src/pages/audit-center.spec.tsx src/pages/operations-console.spec.tsx src/i18n/messages.parity.spec.ts', saRoot);
...
fs.appendFileSync(
  ...
  JSON.stringify({
    ...
    frozenDatabase: env.STEP22_FROZEN_DATABASE_NAME,
    frozenDatabaseUrl: env.STEP22_FROZEN_DATABASE_URL,
  }) + '\n',
);

console.log('\nSTEP 22 FINAL ONE-PASS SEQUENCE COMPLETE', {
  durationMs: seqEnd - seqStart,
  logPath,
  frozenDatabase: env.STEP22_FROZEN_DATABASE_NAME,
});
