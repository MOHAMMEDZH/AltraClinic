...
export function WorkflowsHomePage() {
  ...
  const metrics = data
    ? [
        ...
        { id: 'sla', label: t('workflow.enterprise.slaStatus'), value: slaOk ? '✓' : '!', meta: slaOk ? t('workflow.enterprise.slaOk') : `${data.overdueTasks} ${t('workflow.kpi.overdue')}`, href: '/workflows/tasks?overdue=true', icon: Timer, accent: (slaOk ? 'success' : 'danger') as const },
        ...
        { id: 'auto', label: t('workflow.enterprise.automationRate'), value: data.failedAutomations, meta: t('workflow.kpi.failedAutomations'), href: '/workflows/automation', icon: Zap, accent: (data.failedAutomations > 0 ? 'danger' : 'success') as const },
