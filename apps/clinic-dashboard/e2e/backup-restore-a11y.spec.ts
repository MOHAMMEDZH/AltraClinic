...
test.describe('Phase 43f — Backup/Restore accessibility', () => {
  ...
  test('center overview has no serious a11y violations', async ({ page }) => {
    ...
    const region = page.locator('#backup-restore-region');
    await expect(region).toBeVisible({ timeout: 60_000 });
    const results = await new AxeBuilder({ page }).include('#backup-restore-region').analyze();
