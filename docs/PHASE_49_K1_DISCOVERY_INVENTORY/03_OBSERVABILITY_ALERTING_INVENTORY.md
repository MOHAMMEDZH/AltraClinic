...
| Alerting tests (in-module) | `apps/api/src/modules/observability/tests/dashboards-and-alerting.spec.ts` | **PARTIAL** (unit/product; not prod alert routing) |
...
3. Risk of inventing parallel APM — must reuse Phase 45 + health contributors only.
4. Flag/default-off posture for Observability Center needs explicit production-enablement note (not product redesign).
