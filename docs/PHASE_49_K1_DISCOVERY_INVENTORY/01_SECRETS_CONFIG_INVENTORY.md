...
2. Step 28 scanners prove Release 47 bar — not packaged as Phase 49 production-hardening evidence format.
3. No documented “fail-closed config” matrix for production cutover (partially in Step 29 readiness; not Phase 49-closed).
4. Risk of local `.env` / evidence dirs (e.g. `.ci-evidence`) — process rule exists; Phase 49 needs explicit non-commit / redaction checklist.
